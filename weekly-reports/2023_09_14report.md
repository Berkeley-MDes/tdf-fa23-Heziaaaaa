...
#### :one: : The frist step is to draw out the middle division line and its two tip corners at the two ends of it. The length of the division line is linked to the length of the phone `(phone length / 1.7)`. The two tip corners are always of `90 degrees` to the middle division line and they can also be adjusted according to the thickness of the phone. 
...
#### 2️⃣ : The second step is to rotate the zig-zag line component to the desired angle. The angle is adjusted as `Viewing Angle'. And then make the 2D outlint into 3D volumn and rotate and move the two vertical stands into the right position in relationship to the 3D tilted phone model.
...
# 3️⃣: Laser cut
#### After I finish with the Grasshopper file, I baked two sets of 2D cellphone stand into illustrator file for lasercut: one is of `45 degrees` and the other is of `70 degrees`, and bring them to lasercut. 
...
#### The final step is to assemble the cutout pieces into 3D functional cell phone stand.
